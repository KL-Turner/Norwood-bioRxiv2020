...
if ~isempty(dataSummary) && strcmp(runFromStart,'n') == true
    ...
    multiWaitbar_JK2020('Tracking animal rearing',0,'Color','K'); pause(0.25);
    multiWaitbar_JK2020('Tracking animal motion',0,'Color','K'); pause(0.25);
    % Run analysis and output a structure containing all the analyzed data.
    [AnalysisResults] = AnalyzeData_JK2020(rootFolder);
    multiWaitbar_JK2020('CloseAll');
end
...
for a = 1:length(animalIDs)
    ...
    multiWaitbar_JK2020('Tracking animal rearing','Value',a/length(animalIDs));
end
...
for b = 1:length(animalIDs)
    ...
    multiWaitbar_JK2020('Tracking animal motion','Value',b/length(animalIDs));
end
